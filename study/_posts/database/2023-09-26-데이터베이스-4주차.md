...
## <span style="background-color: #fff5b1">Relational Algebra
</span>
...
- 각 중간 단계의 임시 릴레이션에 이름을 부여할 수도 있음
  - DEPT4_EMPS <- $\sigma$<sub>Dno=4</sub>(EMPLOYEE)
  - R <- $\pi$<sub>Fname, Lname, Salary</sub>(DEPT4_EMPS)
- 결과 릴레이션의 애트리뷰트 이름은 <span style="background-color: #fff5b1">재명명</span>할 수도 있음
  - **DEPT$_EMPS <- $\sigma$<sub>Dno=4</sub>(EMPLOYEE)**<br>
...
- R의 튜플 t는 R<sub>1</sub>의 튜플 t<sub>1</sub>과 R<sub>2</sub>의 튜플 t<sub>2</sub> 분리됨; 즉, <br><br>
<center>t[A<sub>1</sub>, A<sub>2</sub>, .. , A<sub>m</sub>] = t<sub>1</sub>  and  t[B<sub>1</sub>, B<sub>2</sub>, ... , B<sub>n</sub>] = t<sub>2</sub>
<br>
...
- 예제 : 모든 DEPARTMENT 튜플과 그 부서장의 EMPLOYEE 튜플을 조합하라.
  - <center>DEP_EMP <- DEPARTMENT X EMPLOYEE</center>
  - <center>DEPT_MANAGER <- 
  $\sigma$<sub>Mgrssn=Ssn</sub>(DEP_EMP)</center>
...
- <center>EMPNAMES <- 
$\pi$<sub>Fname, Lname, Ssn ($\sigma$<sub>Sex='F'(EMPLOYEE))<br>
EMP_DEPENDENTS <- EMPNAMES X DEPENDENT<br>
ACTUAL_DEPENDENTS <- $\sigma$<sub>Ssn=Essn(EMP_DEPENDENTS)<br>
RESULT <- $\pi$<sub>Fname, Lname, Dependent_name(ACTUAL_DEPENDENTS)<br></center>
...
<br>
<center>DEPT_MGR <- DEPARTMENT 

$\Join$<sub>Mgr_ssn=Ssn EMPLOYEE</center><br>
<center>RESULT <- 

$\pi$<sub>Dname, Fname, Lname (DEPT_MGR)</center>
<br>
...
- <span style="background-color: #fff5b1">EQUIJOIN</span>
  ...
  - EQUIJOIN 사용 예제
    - 모든 DEPARTMENT의 이름과 그 관리자의 이름을 검색하라:<br>
    DEPT_MGR <- DEPARTMENT $\join$<sub>Mgr_ssn=Ssn EMPLOYEE<br>
    RESULT <- $\pi$<sub>Dname, Fname, Lname (DEPT_MGR)
<br><br>
...
- <span style="background-color: #fff5b1">자연 조인 (NATURAL JOIN)(*)</span> :
  ...
  <center>T <- EMPLOYEE *<sub>(Dno),(Dnumber)DEPARTMENT<br>
...
<br>
  ...
  <center>SUPERVISOR(Superssn, SFN, SLN) <- 
  
  $\sigma$<sub>Ssn, Fname, Lname</sub>(EMPLOYEE)<br>
  T <- EMPLOYEE * SUPERVISOR<br>
  RESULT <- $\pi$<sub>Fname,Lname,SFN,SLN(T)</center>
<br><br>
...
<center>SUPERVISOR(Sssn, SFN, SLN) <- 

$\pi$<sub>Ssn, Fname, Lname(EMPLOYEE)<br>
T <- EMPLOYEE $\join$<sub>Superssn = ssn SUPERVISOR<br>
RESULT <- $\pi$<sub>Fname, Lname, SFN, SLN(T)</center>
<br><br>
...
$\div$</span>은 다음과 같이 정의됨 <span style="background-color: #fff5b1">(X $\subseteq$ Z 이고, Y = Z - X임)</span><br>
<center>T<sub>1</sub> =

$\pi$<sub>Y</sub>(R)<br>T<sub>2</sub> = $\pi$<sub>Y</sub>((S X T<sub>1</sub>) - R)<br>T = T<sub>1</sub> - T<sub>2</sub></center>
<br>
